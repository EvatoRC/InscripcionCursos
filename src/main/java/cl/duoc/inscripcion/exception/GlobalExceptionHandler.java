package cl.duoc.inscripcion.exception;
...
import cl.duoc.inscripcion.dto.ErrorResponse;
...
public class GlobalExceptionHandler {
	...
	/**
	 * Maneja excepciones de archivos invalidos
	 */
	...
	/**
	 * Maneja excepciones cuando el archivo excede el tamano maximo permitido
	 */
	...
	public ResponseEntity<ErrorResponse> handleMaxUploadSizeExceededException(MaxUploadSizeExceededException ex,
		...
		ErrorResponse error =
				ErrorResponse.builder().timestamp(LocalDateTime.now()).status(HttpStatus.PAYLOAD_TOO_LARGE.value())
						.error("File Too Large").message("El archivo excede el tamano maximo permitido")
						.details(ex.getMessage()).path(getPath(request)).build();
	...
	public ResponseEntity<ErrorResponse> handleS3Exception(S3Exception ex, WebRequest request) {
		...
		ErrorResponse error = ErrorResponse.builder().timestamp(LocalDateTime.now()).status(status.value())
				.error("S3 Error").message("Error al realizar la operacion en S3")
	...
	/**
	 * Maneja excepciones genericas no capturadas
	 */
